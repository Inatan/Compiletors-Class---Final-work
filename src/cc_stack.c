...
int setLiteralTypeSize(comp_dict_item_t* data_item)
{
	switch(data_item->iks_type)
	{
		case IKS_INT:
		...
		case IKS_CHAR:
			...
			break;
		case IKS_STRING:
			return CHAR_SIZE * strlen(data_item->key)+1;
			break;
	}
}
			...
			if(aux_stack->data->key == data->key)
				{
					...
					{
						strcpy(data->funcLabel,aux_stack->data->funcLabel);
						data->funcOffset= aux_stack->data->funcOffset;
						data->localVarSize=	 aux_stack->data->localVarSize;
					}
				}
			else
				aux_stack = aux_stack->prev;
...
int Function_Comparsion(int chamada,stack_item* stack, stack_item* call_stack)
{
	...
	{   
		if(callParamStack->data!=NULL && mainParamStack!=NULL && callParamStack->data->key!=NULL )
		{
