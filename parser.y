...
declaracao_global
	: especificador_tipo TK_IDENTIFICADOR '[' Array ']' {
															...
															printList($2->array);
															$2->scopeType= globalDeclaration;
														}
	| especificador_tipo TK_IDENTIFICADOR {$2->iks_type = $1;$2->nodeType = AST_IDENTIFICADOR;$2->scopeType= globalDeclaration;setTypeSize($2,NULL,globalDeclaration); stack_push(&main_stack,$2,data_item, 1);}
	| TK_PR_STATIC especificador_tipo  TK_IDENTIFICADOR '[' Array ']' { $3->iks_type = $2;setTypeSize($3,$3->array,globalDeclaration);stack_push(&main_stack,$3,data_item, 1);dimensions = 0; deleteList(&list_dimensions);$3->scopeType= globalDeclaration;}
	;
	...
	: escopo '(' parametros_vazio ')'
								...
								bloco_comando 
									...
									{ 	
										...
										$1->tableItem->localVarSize = localVarsSize;
										localVarsSize=0;
										frameOffset+=frameOffsetParam; // tamanho dos parametros.

										$1->tableItem->funcOffset = frameOffset;
										$$ = $1;
										if($8 !=NULL)
											appendChildNode($$,$8);
										frameOffsetParam=0;
...
escopo
	:especificador_tipo TK_IDENTIFICADOR {Func_type = $1;$2->iks_type = $1; $$ = createNode(AST_FUNCAO,$2);stack_push(&main_stack,$2,func_item, 1);setTypeSize($2,NULL,localDeclaration);
		if(strcmp($2->key,"main")!=0)
			frameOffset+=$2->size;
		$2->isParam = 1;}
	;
...
lista_parametros 
	: especificador_tipo TK_IDENTIFICADOR {$2->iks_type = $1; stack_push(&main_stack,$2,param_item, 1); param++;setTypeSize($2,NULL,localDeclaration); frameOffsetParam+=$2->size;$2->isParam = 1;}
	| TK_PR_CONST especificador_tipo TK_IDENTIFICADOR {$3->iks_type = $2;stack_push(&main_stack,$3,param_item, 1); param++;frameOffsetParam+=$3->size;$3->isParam = 1;}
	;
...
declaracao_local
	: especificador_tipo TK_IDENTIFICADOR   {$$ = NULL;$2->nodeType = AST_IDENTIFICADOR; $2->iks_type = $1;setTypeSize($2,NULL,localDeclaration);stack_push(&main_stack,$2,data_item, 1);frameOffset+=$2->size;localVarsSize+=$2->size;}
	| especificador_tipo TK_IDENTIFICADOR TK_OC_LE valor {$$ = NULL;$2->nodeType = AST_IDENTIFICADOR; $2->iks_type = $1;setTypeSize($2,NULL,localDeclaration);stack_push(&main_stack,$2,data_item, 1);typeCoercion($2,$4->tableItem,0);frameOffset=$2->size;localVarsSize+=$2->size;}
	| TK_PR_STATIC especificador_tipo TK_IDENTIFICADOR {$$ = NULL;$3->nodeType = AST_IDENTIFICADOR; $3->iks_type = $2;setTypeSize($3,NULL,localDeclaration);stack_push(&main_stack,$3,data_item, 1);frameOffset+=$3->size;localVarsSize+=$3->size;}
	| TK_PR_STATIC especificador_tipo  TK_IDENTIFICADOR TK_OC_LE valor {$$ = NULL;$3->nodeType = AST_IDENTIFICADOR; $3->iks_type = $2;setTypeSize($3,NULL,localDeclaration);stack_push(&main_stack,$3,data_item, 1);typeCoercion($3,$5->tableItem,0);frameOffset+=$3->size;localVarsSize+=$3->size;}
	| TK_PR_STATIC TK_PR_CONST especificador_tipo  TK_IDENTIFICADOR TK_OC_LE valor {$$ = NULL;$4->nodeType = AST_IDENTIFICADOR; $4->iks_type = $3;setTypeSize($4,NULL,localDeclaration);stack_push(&main_stack,$4,data_item, 1);typeCoercion($4,$6->tableItem,0);frameOffset+=$4->size;localVarsSize+=$4->size;}
	| TK_PR_CONST especificador_tipo  TK_IDENTIFICADOR TK_OC_LE valor {$$ = NULL;$3->nodeType = AST_IDENTIFICADOR; $3->iks_type = $2;setTypeSize($3,NULL,localDeclaration);stack_push(&main_stack,$3,data_item, 1);typeCoercion($3,$5->tableItem,0);frameOffset+=$3->size;localVarsSize+=$3->size;}
	;
...
chamada_funcao
	: nome  '(' lista_vazia ')' {Function_Comparsion(chamada,main_stack,call_stack); $$ = createNode(AST_CHAMADA_DE_FUNCAO,$1->tableItem);$$->iks_type = $1->iks_type; appendChildNode($$,$1);if($3 != NULL) {appendChildNode($$,$3);}$$->iks_type = getFunctionType(main_stack,call_stack);chamada = 0;}
	;
...
expressao 
	...
	| literal 								{$$ = $1;$$->iks_type = $1->iks_type;$1->tableItem->size=setLiteralTypeSize($1->tableItem);}
